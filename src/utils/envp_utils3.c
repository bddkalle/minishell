...
/*   Updated: 2025/05/03 14:12:28 by vboxuser         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/minishell.h"

int	to_array_quoted(t_vars *vars, char **envp)
{
	int	i;
	t_envp	*temp;
	...
	while (temp)
	{
		...
		if (temp->val_set)
		{
			...
			ft_strlcat(envp[i], "\"", ft_strlen(envp[i]) + 2);
			ft_strlcat(envp[i], temp->value, ft_strlen(envp[i]) + ft_strlen(temp->value) + 1);
			ft_strlcat(envp[i], "\"", ft_strlen(envp[i]) + 2);
		}
		i++;
		temp = temp->next;
